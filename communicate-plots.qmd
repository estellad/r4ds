...
## Saving your plots {#sec-saveplot}

### Saving plots as stationary - ggsave()
...
### Saving plots as editable - officer

Saving you ggplot2 object as an office object in Word or Powerpoint, where you can manually edit all of the downstream analysis.

Using the previous plot as an example:
...
# install.packages(c("officer", "rvg", "grid"))
library(officer)
library(rvg)
library(grid)
...
ppt = read_pptx() %>%
  ...
  ph_with(value = if(is.grob(plot)) dml(grid.draw(plot))
          else dml(ggobj = plot),
          ph_location(left = 0.5, top = 0.5, width = 9, height = 6.5)) %>%
  print(target = "editable_scatter.pptx")
```

The same idea applies to R table objects

<!-- TODO: now insert pptx screenshot -->
